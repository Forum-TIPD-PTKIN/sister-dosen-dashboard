...
$query = $_POST['query_data'];
$count_query = $_POST['count_query_data'];
$alias = array_filter($_POST['alias1']);
$arr_prepare = "";
$data_array_content = array();
$final_data_array_content = "";
$param_init = "";
$filter_block = "";

$button_filter_or_not = '
<button class="btn btn-primary btn-flat download-data-'.$service_name.'-now"><i class="fa fa-cloud-download"></i> Download Data</button>
';
$download_filter_or_not ='
$(".download-data-'.$service_name.'-now").on("click", function() {
    total_down_'.$service_name.'();
});
';
...
if (isset($_POST['is_filter'])) {
    ...
    $filter_where = array();
    $prepare_var = array();
    $filter_where_var_init = array();
    $form_select = array();
    $filter_download_js = array();
    
    foreach ($_POST['is_filter'] as $source_field => $target_field) {
      //$col_name_filter = substr(strrchr($source_field, '.'), 1);
      $filter_where[] = '$and_'.$source_field.' = "and '.$source_field.'=?";';
      $prepare_var[] = '"'.$source_field.'" => $_POST["'.$source_field.'"]';
      $filter_where_var[] = '$and_'.$source_field;
      $filter_where_var_init[] = '$and_'.$source_field.'= "";';

      $filter_download_js[] = $source_field.' : $("#'.$source_field.'_'.$service_name.'").val()';

      $form_select[] = '
        <div class="form-group">
          <label for="Jurusan" class="control-label col-lg-1" style="text-align:right;">'.$source_field.'</label>
          <div class="col-lg-3">
            <select id="'.$source_field.'_'.$service_name.'" name="'.$source_field.'_'.$service_name.'" data-placeholder="Pilih Jurusan ..." class="form-control chzn-select" tabindex="2">
...
$filter_where_query = implode(" and ", $filter_where);

$arr_prepare = ',array('.implode(",", $prepare_var).')
    ';

$filter_var = '
if ($_POST[\''.$source_field.'\']!=\'all\') {
//param here
$param = array(
    '.implode(",\n\t", $prepare_var).'
);
...
';
$param_init = implode("\n", $filter_where_var_init);

    $query .= " where 1=1 ".implode(" ", $filter_where_var);
    $count_query .= " where 1=1 ".implode(" ", $filter_where_var);

$filter_block .= '
<!--start filter block -->
<div id="download-filter-'.$service_name.'" style="display:none">
  <form class="form-horizontal">
    ...
    <div class="form-group">
      ...
      <div class="col-lg-3">
        <span class="btn btn-success btn-flat download-data-'.$service_name.'-now">
...
$button_filter_or_not = '
<button class="btn btn-primary btn-flat download-data-'.$service_name.'"><i class="fa fa-cloud-download"></i> Download Data</button>
';
$download_filter_or_not ='
$(".download-data-'.$service_name.'").on("click", function() {
  if ($("#download-filter-'.$service_name.'").is(":visible")){
    $("#download-filter-'.$service_name.'").hide();
  } else {
    $("#download-filter-'.$service_name.'").show();
  }
});
...
';
 ...
 $no_download = array();
$var_loop = array();
foreach ($_POST['target_field'] as $source => $target) {
  if ($target=='no_download_col') {
    $no_download[] = $source;
  } else {
     $var_loop[] = '"'.$target.'" => $key->'.$source;
  }
}
...
foreach ($alias as $source_alias => $val_alias) {
    if (!in_array($val_alias, $no_download)) {
      $data_array_content[] = '"'.$val_alias.'" => $key->'.$val_alias;
    }
}
$final_data_array_content = implode(",\n\t\t\t", $data_array_content);
...
$db->createFile('../result/get_'.$service_name.'.php',$get_template);
$db->createFile('../result/view_'.$service_name.'.php',$view_template);
$db->createFile('../result/stream_'.$service_name.'.php',$stream_template);
...
foreach ($query as $dt) {
    $query_result[] = array_values($db->convertObjToArray($dt));
}
