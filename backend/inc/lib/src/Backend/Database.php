...
public function mapper($update_value, $primary_key_value) {
    $index = 0;
    $append = []; // make sure initialized

    foreach ($update_value as $value) {
        $primary_value = $primary_key_value[$index];
        $append[] = array_map(function ($str) use ($primary_value) {
            // use PDO quote to handle special characters, JSON, quotes, etc.
            $quotedStr = $this->pdo->quote($str);
            $quotedPrimary = $this->pdo->quote($primary_value);
            return "WHEN {$quotedPrimary} THEN {$quotedStr}";
        }, $value);
        $index++;
    }
    return $append;
...
public function updateMulti($table_name, $update_value, $primary_key_name, $primary_key_value) {
    ...
    // Quote each primary key value
    $primary_key_quote = implode(",", array_map([$this->pdo, 'quote'], $primary_key_value));
 ...
 *
 * @param string $table_name Table name
 * @param array $values Multi-dimensional array of data
 * @return bool Returns true on success, false on failure
 */
public function insertMulti($table_name, $values)
{
    if (empty($values) || !is_array($values)) {
        $this->setErrorMessage('Invalid or empty data provided.');
        return false;
    }
    ...
    // Prepare placeholders for values (e.g., (?, ?, ?, ?, ?, ?))
    $placeholders = '(' . implode(',', array_fill(0, count($column_names), '?')) . ')';
    $placeholders_all = implode(',', array_fill(0, count($values), $placeholders));

    // Build the SQL query
    $sql = "INSERT INTO $table_name ($column_names_str) VALUES $placeholders_all";
    ...
    function convertObjToArray($obj)
    {
        ...
        if (is_array($obj)) {
            ...
            foreach ($obj as $key => $val) {
                $new[$key] = $this->convertObjToArray($val);
            }
        } else {
            $new = $obj;
        }
    ...
    public function roleUserMenu()
    {
        ...
        foreach ($this->query(
            "select sys_menu.url from sys_menu inner join sys_menu_role on sys_menu.id=sys_menu_role.id_menu
          where sys_menu_role.group_level=? and sys_menu_role.read_act=?", array('sys_menu_role.group_level'=>$_SESSION['group_level'],'sys_menu_role.read_act'=>'Y')
        ) as $role) {
            $role_user[]=$role->url;
        }
        echo $this->getErrorMessage();
